...
class line_split:
    ...
    def __init__(self, image, table=TABLE, split_threshold=4):
        self.image = image
        self.table = table
    ...
    def l_split(self):
        ...
        pos = np.where(project <= self.split_threshold)[0]  # split_threshold不能在设置为0了,10左右应该可以，可以自己调
        project[pos] = 0
        # 正反傅里叶，将信号连续化
...
if __name__ == '__main__':
    ...
    line_imgs = line_split(img).l_split()

    tmp = 0
    for line_img in line_imgs:
        cv2.imwrite('./line_split/'+str(tmp)+'.jpg',line_img[0])
        tmp += 1
