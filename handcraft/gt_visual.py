...
def draw_2(coords, image, is_box = True):
    img = image
    img_empty = np.zeros(img.shape, np.uint8)
    img_empty.fill(255)
    for order, item in enumerate(coords.values()):
        ...
        for count, point in enumerate(item):
            if count == 0:
                ...
                cv2.line(img, item[count -1], item[count], (0, 255, 0), 3)
        if is_box:
            cv2.line(img, item[-1], item[0], (0, 255, 0), 3)
...
if __name__ == '__main__':
    ...
    # example
    xml_path = './train/page/0533_001.pdf_18.xml'
    img = './train/imgs/0533_001.pdf_18.png'
